...
void QmitkIGTTrackingLabView::UpdateTimer()
{
  ...
  if (m_CameraView && m_VirtualView.IsNotNull())
  {
...
void QmitkIGTTrackingLabView::OnInstrumentSelected()
{
  if (m_Controls.m_TrackingDeviceSelectionWidget->GetSelectedNavigationDataSource().IsNotNull())
  {
    m_InstrumentNavigationData =
      m_Controls.m_TrackingDeviceSelectionWidget->GetSelectedNavigationDataSource()->GetOutput(
...
void QmitkIGTTrackingLabView::OnInitialRegistration()
{
  ...
  if (m_Controls.m_ImageActive->isChecked() && m_Controls.m_ImageComboBox->GetSelectedNode().IsNotNull())
  {
    ...
    m_T_ImageGeo = mitk::AffineTransform3D::New(); // this is also stored in a member because it is needed for permanent
...
void QmitkIGTTrackingLabView::OnPermanentRegistration(bool on)
{
...

  {
    ...
    // remember initial object transform to calculate the object to marker transform later on and convert it to
    // navigation data
    mitk::AffineTransform3D::Pointer transform =
      this->m_Controls.m_ObjectComboBox->GetSelectedNode()->GetData()->GetGeometry()->GetIndexToWorldTransform();
    mitk::NavigationData::Pointer T_Object =
      mitk::NavigationData::New(transform, false); // TODO: catch exception during conversion?
    ...
    {
      this->m_Controls.m_ImageComboBox->GetSelectedNode()->GetData()->GetGeometry()->SetIndexToWorldTransform(
        m_T_ImageGeo);
    }
    ...
    m_PermanentRegistrationFilter->SetRepresentationObject(
      0, this->m_Controls.m_ObjectComboBox->GetSelectedNode()->GetData());
    ...
    if (m_Controls.m_ImageActive->isChecked() && (m_Controls.m_ImageComboBox->GetSelectedNode().IsNotNull()))
    {
      ...
      // for the image we can't use NavigationData objects as transforms because an image needs additional geometry
      // information, e.g., spacing thus we use mitk::AffineTransform3D objects
  ...
  else // if off = disable the permanent registration
  {
    ...
    {
      this->m_Controls.m_ObjectComboBox->GetSelectedNode()->GetData()->GetGeometry()->SetIndexToWorldTransform(
        m_T_ObjectReg->GetAffineTransform3D());
    }
    if (m_T_ImageReg.IsNotNull())
    {
      this->m_Controls.m_ImageComboBox->GetSelectedNode()->GetData()->GetGeometry()->SetIndexToWorldTransform(
        m_T_ImageReg);
    }
...
void QmitkIGTTrackingLabView::OnVirtualCamera(bool on)
{
  ...
  if (on)
  {
    ...
    viewUpVector[0] = m_Controls.m_viewX->value(); //(int)(m_Controls.m_NeedleUpX->isChecked());
    viewUpVector[1] = m_Controls.m_viewY->value(); //(int)(m_Controls.m_NeedleUpY->isChecked());
    viewUpVector[2] = m_Controls.m_viewZ->value(); //(int)(m_Controls.m_NeedleUpZ->isChecked());
    if (m_Controls.m_NeedleUpInvert->isChecked())
      viewUpVector *= -1;
...
void QmitkIGTTrackingLabView::CreateConnections()
{
  ...
  connect(
    m_Controls.m_RegistrationWidget, SIGNAL(AddedTrackingFiducial()), this, SLOT(OnAddRegistrationTrackingFiducial()));
...
bool QmitkIGTTrackingLabView::CheckRegistrationInitialization()
{
  ...
  else if ((imageFiducials->GetSize() < 3) || (trackerFiducials->GetSize() < 3) ||
           (imageFiducials->GetSize() != trackerFiducials->GetSize()))
  {
    warningMessage = "Not enough fiducial pairs found. At least 3 fiducial must exist for the image and the tracking "
                     "system respectively.";
    initializationErrorDetected = true;
  }
...
bool QmitkIGTTrackingLabView::IsTransformDifferenceHigh(mitk::NavigationData::Pointer transformA,
                                                        ...
                                                        double euclideanDistanceThreshold,
                                                        double angularDifferenceThreshold)
{
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      rotMatrixA[i][j] = rotA.rotation_matrix_transpose().transpose()[i][j];

  itk::Matrix<double, 3, 3> rotMatrixB;
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
      rotMatrixB[i][j] = rotB.rotation_matrix_transpose().transpose()[i][j];

  itk::Vector<double, 3> pt1 = rotMatrixA * point;
  itk::Vector<double, 3> pt2 = rotMatrixB * point;

  returnValue = (pt1[0] * pt2[0] + pt1[1] * pt2[1] + pt1[2] * pt2[2]) /
                (sqrt(pow(pt1[0], 2.0) + pow(pt1[1], 2.0) + pow(pt1[2], 2.0)) *
                 sqrt(pow(pt2[0], 2.0) + pow(pt2[1], 2.0) + pow(pt2[2], 2.0)));
